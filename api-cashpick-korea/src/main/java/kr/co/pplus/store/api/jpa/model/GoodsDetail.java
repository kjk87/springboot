...
@Entity(name="goodsDetail") // This tells Hibernate to make a table out of this class
@Table(name="goods")
@Data
@AllArgsConstructor
@NoArgsConstructor
...
//    @Formula("(select count(1) from goods_review gr where gr.goods_seq_no = seq_no)")
    Long reviewCount = 0L ;

    @Column(name="avg_eval", insertable = false, updatable = false)
//    @Formula("(select avg(gr.eval) from goods_review gr where gr.goods_seq_no = seq_no)")
    Double avgEval = 0.0 ;

    @Column(name="like_count", insertable = false, updatable = false)
//    @Formula("(select count(1) from goods_like gl where gl.goods_seq_no = seq_no and gl.status = 1)")
    Long likeCount = 0L ;

    @Column(name="distance", insertable = false, updatable = false)
    Double distance = 0.0 ;

//    @Column(name="page_category_seq_no", insertable = false, updatable = false)
