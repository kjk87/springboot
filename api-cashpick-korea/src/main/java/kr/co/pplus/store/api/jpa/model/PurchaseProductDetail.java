...
@Data
@Entity(name="purchaseProductDetail")
...
public class PurchaseProductDetail {
    ...
    @JoinColumn(name = "purchase_product_seq_no", insertable = false, updatable = false)
    private List<PurchaseProductOption> purchaseProductOptionList = null;

    @Key
    @ManyToOne(fetch = FetchType.EAGER, cascade = CascadeType.ALL)
    @JoinColumn(name = "purchase_delivery_seq_no", referencedColumnName = "seq_no", insertable = false, updatable = false)
    PurchaseDelivery purchaseDelivery = null;

    @Key
    @ManyToOne(fetch = FetchType.EAGER, cascade = CascadeType.ALL)
    @JoinColumn(name = "purchase_seq_no", referencedColumnName = "seq_no", insertable = false, updatable = false)
    Purchase purchase = null; // '구매 정보',
    ...
    @Key
    @ManyToOne(fetch = FetchType.EAGER, cascade = CascadeType.ALL)
    @JoinColumn(name = "product_price_code", referencedColumnName = "code", insertable = false, updatable = false)
    ...
    @Formula("(select count(1) from product_review pr where pr.purchase_product_seq_no = seq_no) > 0")
