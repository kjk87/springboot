...
@Data
@Entity(name = "luckyPickPurchaseItem")
@Table(name = "lucky_pick_purchase_item")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class LuckyPickPurchaseItem implements Serializable {
    ...
    @Column(name="lucky_pick_purchase_seq_no")
    private Long luckyPickPurchaseSeqNo;
    @Column(name="lucky_pick_pay_response_tran_seq")
    private String luckyPickPayResponseTranSeq; // 럭키픽 구매시 거래번호:취소요청시 사용값
    ...
    @Column(name="lucky_pick_title")
    private String luckyPickTitle;
    ...
    @Column(name="lucky_pick_delivery_purchase_seq_no")
    private Long luckyPickDeliveryPurchaseSeqNo;
    @Column(name="delivery_payment_price")
    private Float deliveryPaymentPrice; // deliveryFee + optionPrice

    @Column(name="lucky_pick_delivery_seq_no")
    private Long luckyPickDeliverySeqNo;

    @Formula("(select count(1) from lucky_pick_review lrv where lrv.lucky_pick_purchase_item_seq_no = seq_no) > 0")
    private Boolean isReviewExist = false;

    @Formula("(select count(1) from lucky_pick_reply lr where lr.lucky_pick_purchase_item_seq_no = seq_no and lr.status = 1)")
    ...
    @JoinColumn(name = "lucky_pick_delivery_purchase_seq_no",  referencedColumnName="seq_no", insertable = false, updatable = false)
    LuckyPickDeliveryPurchaseRef luckyPickDeliveryPurchase = null ;

    @NotFound(action = NotFoundAction.IGNORE)
    @OneToOne(fetch=FetchType.EAGER, cascade = CascadeType.ALL)
    @JoinColumn(name = "lucky_pick_delivery_seq_no",  referencedColumnName="seq_no", insertable = false, updatable = false)
    LuckyPickDelivery luckyPickDelivery = null ;
